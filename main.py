...
def create_ad():
    if request.method == 'POST':
        ...
        try:
            contract.functions.createAd(int(es_id), w3.to_wei(price, 'ether'))
            return redirect(url_for('main_page'))
        except ValueError as e:
            errors(e)
...
def change_es():
    if request.method == 'POST':
        ...
        try:
            contract.functions.updateEstateStatus(int(es_id), bool(is_active)).transact({'from': login})
            return redirect(url_for('main_page'))
        except ValueError as e:
            errors(e)
...
def change_ad():
    if request.method == 'POST':
        ...
        is_active = request.form.get("is_active")
        try:
            contract.functions.updateAdStatus(int(ad_id), bool(int(is_active))).transact({'from': login})
            return redirect(url_for('main_page'))
        except ValueError as e:
            errors(e)
...
def buy():
    ...
    if request.method == 'POST':
        ...
        try:
            contract.functions.buyEstate(int(es_id), int(ad_id)).transact({'from': login, 'value': value, 'gasPrice': w3.to_wei('10', 'ether')})
            return redirect(url_for('main_page'))
        except ValueError as e:
            errors(e)
